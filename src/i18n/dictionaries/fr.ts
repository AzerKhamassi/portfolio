import type { Dictionary } from "@/i18n/dictionary";

const fr: Dictionary = {
  meta: {
    title: "Azer Khamassi | Développeur Full Stack",
    description:
      "Développeur Full Stack avec 5+ ans d'expérience sur des applications web réactives et évolutives avec React, Node.js et TypeScript. Spécialisé en architecture de composants et optimisation des performances UI.",
    jobTitle: "Développeur Full Stack",
    keywords: [
      "Azer Khamassi",
      "Développeur Full Stack",
      "Développeur Frontend",
      "Développeur React",
      "Développeur Next.js",
      "TypeScript",
      "Node.js",
      "Ingénieur logiciel Tunisie",
      "Architecture de composants",
      "Optimisation des performances UI",
    ],
  },
  nav: {
    work: "projets",
    about: "à propos",
    experience: "expérience",
    education: "formation",
    contact: "contact",
  },
  hero: {
    eyebrow: "// bonjour le monde",
    role: "Développeur Full Stack",
    stack: "React, Node.js, TypeScript",
    location: "Sousse, Tunisie",
    description:
      "5+ ans à concevoir des applications web réactives et évolutives, de bout en bout. Je construis des API REST et des modèles de données relationnelles côté backend, puis je développe des interfaces à base de composants avec un souci constant de l'optimisation des performances UI. Je travaille à distance avec des équipes internationales, j'encadre des développeurs juniors, et je garde un code propre et maintenable.",
    ctaContact: "contacter →",
    ctaWork: "projets",
    ctaTerminal: "ouvrir le terminal",
  },
  projects: {
    sectionLabel: "01 / projets",
    items: [
      {
        title: "IconDesk",
        year: "2023",
        description:
          "Plateforme SaaS B2B qui unifie les analytics Instagram, TikTok et YouTube dans un tableau de bord unique pour les agences de talents. Elle automatise le suivi des performances et transforme des données éparses en media kits prêts à présenter.",
        tags: ["React", "Vite", "Node.js", "Express", "TypeScript", "PostgreSQL", "Stripe"],
        href: "https://www.icondesk.co",
      },
      {
        title: "The Platform TV",
        year: "2022",
        description:
          "Plateforme de streaming vidéo qui permet aux créateurs et aux entreprises de lancer des services d'abonnement à leur propre marque, combinant VOD, streaming en direct et paiements intégrés.",
        tags: ["React", "Node.js", "Express", "TypeScript", "PostgreSQL", "Agora SDK", "MyFatoorah"],
        href: "https://www.theplatform-tv.com",
      },
    ],
  },
  about: {
    sectionLabel: "02 / à propos",
    paragraphs: [
      "Je suis Développeur Full Stack et je conçois des applications web réactives et évolutives en télétravail chez Plantec, où j'ai fiabilisé le code par un typage TypeScript strict, construit une bibliothèque réutilisable de plus de 100 composants, et livré des fonctionnalités du schéma PostgreSQL jusqu'à l'écran final : paiements Stripe, notifications temps réel via Server-Sent Events.",
      "Auparavant, j'ai travaillé sur des microservices NestJS chez PixiMind et développé des fonctionnalités temps réel avec Socket.IO chez Karwisoft. J'ai passé l'essentiel de ma carrière au sein d'équipes distribuées et internationales, et je porte une attention particulière à l'optimisation des performances UI, à la couverture de tests, et à l'encadrement. J'ai accompagné des développeurs juniors en revue de code sur React et TypeScript.",
    ],
    stackLabel: "stack",
    stack: [
      "React.js",
      "Next.js",
      "TypeScript",
      "Redux Toolkit",
      "TailwindCSS",
      "Node.js",
      "Express",
      "NestJS",
      "PostgreSQL",
      "MongoDB",
      "API REST",
      "Socket.IO",
      "Docker",
    ],
  },
  experience: {
    sectionLabel: "03 / expérience",
    items: [
      {
        role: "Développeur Full Stack",
        company: "Plantec",
        period: "07/2022 – Présent",
        location: "Télétravail",
        achievements: [
          "Concevoir une bibliothèque de plus de 100 composants réutilisables, à l'architecture claire et évolutive.",
          "Rédiger une couverture complète de tests unitaires, réduisant les bugs de régression de 30%.",
          "Optimiser les performances de l'application de 25% via le lazy loading et le code splitting.",
          "Développer un système de notifications temps réel (SSE) et de rappels planifiés via des tâches cron.",
          "Encadrer 2 développeurs juniors : revues de code, montée en compétences React et TypeScript.",
        ],
        tools: [
          "React.js",
          "TypeScript",
          "Redux Toolkit",
          "RTK Query",
          "Node.js",
          "PostgreSQL",
          "AWS",
          "TailwindCSS",
          "i18next",
          "Vitest",
        ],
      },
      {
        role: "Développeur Full Stack",
        company: "PixiMind",
        period: "01/2022 – 06/2022",
        location: "Sfax, Tunisie",
        achievements: [
          "Développer des microservices NestJS exposant et consommant des API RESTful.",
          "Structurer des micro-frontends modulaires basés sur l'Atomic Design pour une UI cohérente et évolutive.",
          "Refactoriser plus de 20 composants class-based hérités en composants fonctionnels modernes.",
        ],
        tools: ["React.js", "NestJS", "MongoDB", "Redux", "Bootstrap"],
      },
      {
        role: "Développeur Full Stack",
        company: "Karwisoft",
        period: "07/2021 – 12/2021",
        location: "Sousse, Tunisie",
        achievements: [
          "Livrer et maintenir un ensemble d'API RESTful avec Node.js et Express.js.",
          "Introduire des fonctionnalités temps réel avec Socket.IO en remplacement des rafraîchissements manuels.",
          "Poser les fondations techniques front-end répondant aux exigences du projet.",
        ],
        tools: ["React.js", "Node.js", "Express", "MongoDB", "Socket.IO", "Context API"],
      },
    ],
  },
  education: {
    sectionLabel: "04 / formation",
    degree: "Diplôme National d'Ingénieur en Génie Logiciel",
    school: "Institut Supérieur des Sciences Appliquées et de Technologie de Sousse",
    period: "09/2017 – 07/2022",
    certificatesLabel: "certificats",
    certificates: [
      { name: "Advanced React by Meta", issuer: "Coursera" },
      { name: "AWS Certified Cloud Practitioner", issuer: "Amazon Web Services" },
      { name: "Scrum Fundamentals", issuer: "SCRUMstudy" },
      { name: "Docker Essentials", issuer: "Cognitive Class" },
    ],
  },
  contact: {
    sectionLabel: "05 / contact",
    intro: "Un projet en tête, ou juste envie de dire bonjour ? Laissez-moi un message ci-dessous.",
    nameLabel: "nom",
    namePlaceholder: "Jean Dupont",
    emailLabel: "email",
    emailPlaceholder: "jean.dupont@example.com",
    messageLabel: "message",
    messagePlaceholder: "Qu'avez-vous en tête ?",
    send: "envoyer →",
    sending: "envoi…",
    success: "Message envoyé. Je vous répondrai bientôt.",
    errors: {
      nameTooShort: "Trop court",
      emailInvalid: "Entrez un email valide",
      messageTooShort: "Dites-m'en un peu plus",
      invalidInput: "Entrée invalide",
      notConfigured: "Le formulaire de contact n'est pas encore configuré.",
      sendFailed: "Impossible d'envoyer votre message. Veuillez réessayer.",
    },
  },
  footer: {
    email: "email",
    github: "github",
    linkedin: "linkedin",
    backToTop: "Retour en haut",
  },
  theme: {
    switchToLight: "Passer en mode clair",
    switchToDark: "Passer en mode sombre",
  },
  email: {
    subject: "Nouveau message de {name}",
    preview: "Nouveau message de {name} via votre portfolio",
    heading: "Nouveau message du formulaire de contact",
    fromLabel: "De",
    messageLabel: "Message",
    footerNote: "Envoyé depuis le formulaire de contact de votre portfolio.",
  },
  terminal: {
    prompt: "invite@azer",
    intro: "Tapez 'help' pour voir les commandes disponibles.",
    helpIntro: "Commandes disponibles :",
    whoami: "Azer Khamassi — Développeur Full Stack. Vous avez trouvé le terminal.",
    azer: "Oui, c'est moi. Merci d'avoir tapé mon nom.",
    themeToggled: "→ passage en mode {theme}",
    themeLight: "clair",
    themeDark: "sombre",
    navigating: "→ direction {section}",
    opening: "→ ouverture de {label}…",
    dateLabel: "Nous sommes le {date}",
    pwd: "/home/invite/portfolio-azer-khamassi",
    ls: "about/  work/  experience/  education/  contact/",
    sudo: "Permission refusée : gentil essai, mais il n'y a pas de root ici.",
    unknownCommand: "commande introuvable : {command} — tapez 'help' pour la liste des commandes",
    commands: {
      help: "liste les commandes disponibles",
      whoami: "qui suis-je ?",
      azer: "dire bonjour",
      pwd: "afficher le répertoire courant",
      ls: "lister les sections",
      about: "aller à la section à propos",
      work: "aller à la section projets",
      experience: "aller à la section expérience",
      education: "aller à la section formation",
      contact: "aller à la section contact",
      theme: "basculer le thème clair / sombre",
      clear: "effacer le terminal",
      github: "ouvrir mon profil GitHub",
      linkedin: "ouvrir mon profil LinkedIn",
      email: "ouvrir votre client email",
      date: "afficher la date du jour",
      sudo: "essayez pour voir",
    },
  },
};

export default fr;
